...
<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-800 md:text-3xl">Kelola Galeri</h1>
            <p class="mt-1 text-sm text-slate-500">Kelola galeri gambar restoran dan momen pelanggan.</p>
        </div>
        <a href="{{ route('admin.galeri.create') }}" class="btn-admin"><i class="fas fa-plus mr-1"></i>Tambah Galeri</a>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="group flex items-center justify-between rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/80 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-orange-500/10 hover:ring-orange-200">
            <div>
                <p class="text-xs font-medium uppercase tracking-wider text-slate-400">Total Galeri</p>
                <p class="mt-1 text-3xl font-bold text-slate-800">{{ $galeris->count() }}</p>
            </div>
        ...
        <div class="group flex items-center justify-between rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/80 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-500/10 hover:ring-emerald-200">
            <div>
                ...
                <p class="mt-1 text-3xl font-bold text-slate-800">{{ $galeris->where('is_active', true)->count() }}</p>
            </div>
        ...
        <div class="group flex items-center justify-between rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/80 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-amber-500/10 hover:ring-amber-200">
            <div>
                ...
                <p class="mt-1 text-3xl font-bold text-slate-800">{{ $galeris->where('is_active', false)->count() }}</p>
            </div>
    ...
    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-slate-700">Daftar Galeri</span>
                <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-500"><i class="fas fa-database"></i>{{ $galeris->count() }} Total</span>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="group transition-all duration-200 hover:bg-slate-50/60">
                        <td class="px-6 py-4 align-middle">
                            @if($g->image)
                                <img src="{{ asset('storage/' . $g->image) }}" alt="{{ $g->title }}" class="h-16 w-16 rounded-xl object-cover ring-1 ring-slate-200">
                            @else
                        ...
                        <td class="px-6 py-4 align-middle">
                            <h6 class="mb-0 text-sm font-semibold leading-normal text-slate-700">{{ $g->title }}</h6>
                        </td>
                        <td class="px-6 py-4 align-middle">
                            <p class="mb-0 text-sm text-slate-500 max-w-[250px] truncate">{{ $g->description ?? '-' }}</p>
                        </td>
                        <td class="px-6 py-4 align-middle">
                            <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $g->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                {{ $g->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 align-middle">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.galeri.edit', $g) }}" class="btn-admin-secondary">Edit</a>
                                <form action="{{ route('admin.galeri.destroy', $g) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus galeri ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="mx-auto max-w-sm text-slate-500">
                                <i class="fas fa-image mb-3 text-2xl text-slate-300"></i>
                                <p class="mb-1 font-semibold text-slate-600">Galeri belum tersedia</p>
                                <p class="mb-0 text-sm">Tambahkan galeri pertama untuk mulai menampilkan gambar restoran.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
