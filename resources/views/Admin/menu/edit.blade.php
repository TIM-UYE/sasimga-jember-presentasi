        ...
        <form action="{{ route('admin.menu.update', $menu) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex flex-col gap-2 rounded-xl border border-orange-200 bg-orange-50 px-4 py-2.5 text-sm text-orange-700">
                        <div class="flex items-center gap-2">
                            <i class="fas fa-calculator"></i>
                            <span>Stok dihitung otomatis dari bahan baku</span>
                        </div>
                        @if(isset($stockCalc) && $stockCalc['stock'] > 0)
                            <div class="flex items-center gap-2 font-semibold">
                                <i class="fas fa-boxes"></i>
                                <span>Stok tersedia: <strong>{{ $stockCalc['stock'] }} porsi</strong></span>
                            </div>
                        @else
                            <div class="flex items-center gap-2 text-red-600">
                                <i class="fas fa-exclamation-triangle"></i>
                                <span>Tambahkan bahan baku untuk menghitung stok otomatis</span>
                            </div>
                        @endif
                    </div>
            ...
            <div class="mb-4 rounded-2xl border border-orange-100 bg-orange-50/40 p-4">
                ...
                <div id="bahan-wrapper" class="space-y-3">
                    @forelse($menu->komposisiBahan as $menuBahan)
                        <div class="grid grid-cols-1 gap-3 rounded-xl bg-white p-3 ring-1 ring-slate-200 md:grid-cols-12">
                            <div class="md:col-span-4">
                                ...
                                <select name="bahan_stok_id[]" onchange="autoFillSatuan(this)"
                                    ...
                                    @foreach($stoks as $stok)
                                        <option value="{{ $stok->id }}" data-satuan="{{ $stok->satuan }}" {{ old('bahan_stok_id', $menuBahan->stok_id) == $stok->id ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-3">
                                ...
                                <input type="number" step="0.01" name="jumlah_dibutuhkan[]"
                                    value="{{ old('jumlah_dibutuhkan.' . $loop->index, $menuBahan->jumlah_dibutuhkan) }}"
                            ...
                            <div class="md:col-span-3">
                                ...
                                <input type="text" name="satuan_bahan[]" readonly
                                    ...
                                    placeholder="Otomatis" value="{{ $menuBahan->satuan ?? 'gram' }}">
                            </div>
            ...
            <div class="mb-4">
                ...
                <div id="image-preview" class="mt-2 {{ $menu->gambar ? '' : 'hidden' }}">
                    @if($menu->gambar)
                        <img id="preview-img" src="{{ asset('storage/menu/' . $menu->gambar) }}" alt="Preview" class="h-48 w-48 rounded-xl object-cover ring-1 ring-slate-200">
                    @else
