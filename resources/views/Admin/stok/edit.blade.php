...
<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-800 md:text-3xl">Edit Stok Bahan</h1>
            <p class="mt-1 text-sm text-slate-500">Perbarui jumlah stok, satuan, dan batas minimum bahan.</p>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80">
        <div class="border-b border-slate-100 px-6 py-4">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-slate-700">Form Edit Bahan</span>
            </div>
        </div>

        <form action="{{ route('admin.stok.update', $stok) }}" method="POST" class="space-y-5 p-6">
            @csrf
            @method('PUT')

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Nama Bahan</label>
                <input type="text" name="nama_bahan" value="{{ old('nama_bahan', $stok->nama_bahan) }}"
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                <div>
                    ...
                    <input type="number" step="0.01" name="jumlah_stok" value="{{ old('jumlah_stok', $stok->jumlah_stok) }}"
                        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="stok_minimum" value="{{ old('stok_minimum', $stok->stok_minimum) }}"
                        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100"
