...
<div class="space-y-6">
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-800 md:text-3xl">Edit Halaman Informasi</h1>
            <p class="mb-0 text-sm text-slate-500">Perbarui informasi: <span class="font-semibold text-orange-600">{{ $information->title }}</span></p>
        </div>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200/80">
        <form action="{{ route('admin.information.update', $information) }}" method="POST" id="informationForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6 border-b border-slate-100 pb-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div class="mb-4">
                        ...
                        <select name="slug" id="slug"
                            ...
                            @foreach ($types as $key => $type)
                                <option value="{{ $key }}" {{ $information->slug == $key ? 'selected' : '' }}>
                                    {{ $type['label'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $information->title) }}"
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-200"
                            placeholder="Judul halaman informasi" required>
                    </div>
                </div>
            </div>
            ...
            <div id="formSections">
                <div class="form-type" data-type="about" style="display:none;">
                    @include('admin.information.forms.about', ['icons' => $icons, 'data' => $parsedContent, 'information' => $information])
                </div>
                <div class="form-type" data-type="faq" style="display:none;">
                    @include('admin.information.forms.faq', ['icons' => $icons, 'routes' => $routes, 'data' => $parsedContent, 'information' => $information])
                </div>
                <div class="form-type" data-type="privacy-policy" style="display:none;">
                    @include('admin.information.forms.privacy', ['icons' => $icons, 'routes' => $routes, 'data' => $parsedContent, 'information' => $information])
                </div>
                <div class="form-type" data-type="terms-conditions" style="display:none;">
                    @include('admin.information.forms.terms', ['icons' => $icons, 'routes' => $routes, 'data' => $parsedContent, 'information' => $information])
                </div>
            </div>
            ...
            <div class="mt-6 flex justify-end gap-2 border-t border-slate-100 pt-5">
                ...
                <button type="submit" class="btn-admin"><i class="fas fa-save mr-1"></i>Update Halaman</button>
            </div>
        </form>
    </div>
</div>
