...
{{-- ITEMS REPEATER --}}
<div class="mb-6 rounded-xl border border-slate-200 bg-slate-50/50 p-5">
    <h4 class="mb-4 text-sm font-bold text-slate-700"><i class="fas fa-list text-orange-500 mr-2"></i>Daftar Konten</h4>
    <div id="privacyItemsContainer">
        @php
            $items = $d['items'] ?? [];
        ...
        @forelse ($loopItems as $i => $val)
            ...
            <div class="privacy-item mb-4 rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-start gap-4">
                    ...
                    <button type="button" onclick="this.closest('.privacy-item').remove()"
    ...
    <button type="button" onclick="addPrivacyItem()" class="btn-admin-secondary text-sm"><i class="fas fa-plus mr-1"></i>Tambah Konten</button>
</div>
...
function addPrivacyItem() {
    const container = document.getElementById('privacyItemsContainer');
        ...
        <div class="privacy-item mb-4 rounded-xl border border-slate-200 bg-white p-4">
            <div class="flex items-start gap-4">
                <div class="flex-1 grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <div>
                        <label class="mb-2 block text-xs font-semibold text-slate-600">Judul</label>
                        <input type="text" name="items_title[]" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-200" placeholder="Judul konten">
                    </div>
                    <div>
                        <label class="mb-2 block text-xs font-semibold text-slate-600">Deskripsi</label>
                        <input type="text" name="items_description[]" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-200" placeholder="Deskripsi konten">
                    </div>
                </div>
                <button type="button" onclick="this.closest('.privacy-item').remove()" class="btn-admin-danger mt-6 px-3 py-1.5 text-xs"><i class="fas fa-trash"></i></button>
            </div>
        </div>
    `);
}
